...
import {ocpiTransactionRemoteStartFun} from '../../services/ocpi_transaction_service';

export default function OcpiChargerInfo({navigation, route}) {
  const {
    stationInfoData,
    station_id,
    station_charger_id,
  ...
  const startCharging = async () => {
    ...
    try {
      ...
      let remoteStartData = {
        mst_emsp_id: stationInfoData.mst_emsp_id,
      ...
      const response = await ocpiTransactionRemoteStartFun(remoteStartData);

      if (response && response.success) {
        var responseData = response.data;
        if (responseData && responseData.ocpi_emsp_transaction_id) {
          await AsyncStorage.setItem('isCharginingStarted', 'true');
          await AsyncStorage.setItem(
            'ocpi_emsp_transaction_id',
            '' + responseData.ocpi_emsp_transaction_id,
          );
  ...
  const goToStatusScreen = async (
    ...
    navigation.replace('OcpiChargingStatus', {
      type_route: type,
        ...
        <LinearGradient
          ...
          <ScrollView
            ...
            <Appbar className="bg-transparent" mode="center-aligned">
              ...
              />
              <Appbar.Content color={'black'} title="Charger Info" />
            </Appbar>

            <View className="flex-1 px-5">
              {chargerInfo()}

              <View className={'mt-1 bg-slate-200 p-3 rounded-xl flex-row'}>
                <Text className="flex-1">
                  Find out how much energy you need to fully charge your vehicle
                </Text>
                <Button
                  onPress={() => {
        ...
        <Surface mode="flat" className="bg-slate-200 rounded-xl p-3 mt-1">
          <View className="flex-1 flex-row justify-between items-center">
            ...
            <Text variant="titleSmall">
              {selected_connector_data.charger_charges} Rs/kWh
            </Text>
          </View>
    ...
    return (
      <Surface
        ...
        <View className="ml-2 w-[96%]">
          <Text variant="titleSmall">
            {stationInfoData.station_name || stationInfoData.name}
          </Text>
        </View>
      </Surface>
    );
