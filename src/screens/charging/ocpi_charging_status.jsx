...
import {
  ocpiTransactionRemoteStopFun,
  ocpiTransactionSessionInfoFun,
} from '../../services/ocpi_transaction_service';

export default function OcpiChargingStatus({navigation, route}) {
  ...
  const fetchSessionData = async () => {
    ...
    try {
      ...
      let sessionInfoData = {
        ...
        ocpi_emsp_transaction_id: transaction_id_temp,
      };
      const response = await ocpiTransactionSessionInfoFun(sessionInfoData);
        ...
        if (is_session_over === 1) {
          ...
          if (is_session_over === 3) {
            ...
            if (is_session_over === 2) {
              ...
              let start_dt = sessionData.session_info_start_datetime;
              ...
              let chargingPercent = JSON.parse(
                sessionData.session_charging_periods,
              );

              let dimensions = chargingPercent[0].dimensions;

              for (let index = 0; index < dimensions.length; index++) {
                const element = dimensions[index];

                if (element.type == 'STATE_OF_CHARGE') {
                  setChargingPercent(parseFloat(element.volume));
                }
              }
              ...
              setcurrent_kwh(sessionData.session_kwh);
              setStationName(sessionData.name);
              ...
              setChargerType(sessionData.power_type == 'DC' ? 'DC' : 'AC');
            }
          }
        }
      } else {
  ...
  async function SwipeSuccess() {
    ...
    try {
      ...
      const response = await ocpiTransactionRemoteStopFun(remoteStopData);
  ...
  return (
    <Animated.View
      ...
      <Appbar className="bg-transparent" mode="center-aligned">
        ...
        />
        <Appbar.Content color="black" title={'Charging'} />
      </Appbar>
      <ScrollView
        ...
        {(() => {
          if (isRewardEnable === true && current_kwh >= transaction_min_unit) {
            return coinCard();
          }
        })()}
  ...
  function TimeRender() {
    c; /* onst voltage = parseFloat(sessionData?.Voltage || 0);
    const current = parseFloat(sessionData?.Current || 0);
    const speed = ((voltage * current) / 1000).toFixed(2); */
